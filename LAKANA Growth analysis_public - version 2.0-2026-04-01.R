...
# Model comparison using Kenward-Roger
anova(mod_WAZ, null_mod_WAZ, ddf = "Kenward-Roger")

# Estimated marginal means with KR df
emm_WAZ <- emmeans(mod_WAZ, ~ intv, lmer.df = "Kenward-Roger")
summary(emm_WAZ, infer = TRUE)

# Treatment vs control contrast with KR df
contr_WAZ <- contrast(
  emm_WAZ,
  method = "trt.vs.ctrl",
  ref = "intv1"
)
summary(contr_WAZ, infer = TRUE)
...
summary(mod_LAZ)
anova(mod_LAZ, null_mod_LAZ,ddf = "Kenward-Roger")

emm_LAZ <- emmeans(mod_LAZ, ~ intv,lmer.df = "Kenward-Roger")
emm_LAZ
summary(emm_LAZ, infer = TRUE)
...
# Summary and model comparison
summary(mod_WLZ)
anova(mod_WLZ, null_mod_WLZ,ddf = "Kenward-Roger")


emm_WLZ <- emmeans(mod_WLZ, ~ intv,lmer.df = "Kenward-Roger")
emm_WLZ
summary(emm_WLZ, infer = TRUE)
...
# Summary and model comparison
summary(mod_MUACZ)
anova(mod_MUACZ, null_mod_MUACZ,ddf = "Kenward-Roger")


emm_MUACZ <- emmeans(mod_MUACZ, ~ intv,lmer.df = "Kenward-Roger")
emm_MUACZ
summary(emm_MUACZ, infer = TRUE)

contr_MUACZ <- contrast(
  emm_MUACZ,
  method = "trt.vs.ctrl",
  ref = "intv1"  # placebo
)

summary(contr_MUACZ, infer = TRUE)
...
summary(mod_weight)
anova(mod_weight, null_mod_weight,ddf = "Kenward-Roger")


emm_weight <- emmeans(mod_weight, ~ intv,lmer.df = "Kenward-Roger")
emm_weight
summary(emm_weight, infer = TRUE)

contr_weight <- contrast(
  emm_weight,
  method = "trt.vs.ctrl",
  ref = "intv1"  # placebo
)

summary(contr_weight, infer = TRUE)
...
summary(mod_length)
anova(mod_length, null_mod_length,ddf = "Kenward-Roger")

emm_length <- emmeans(mod_length, ~ intv,lmer.df = "Kenward-Roger")
emm_length
summary(emm_length, infer = TRUE)

contr_length <- contrast(
  emm_length,
  method = "trt.vs.ctrl",
  ref = "intv1"  # placebo
)

summary(contr_length, infer = TRUE)
...
growth %>%
  group_by(intv) %>%
  summarise(
    Mean = round(mean(MUACZ, na.rm = TRUE), 2),
    SD = round(sd(MUACZ, na.rm = TRUE), 2)
  )
...
anova(null_mod, mod_stunting, test = "Chisq",ddf = "Kenward-Roger")
...
# ------------------------------
# Get adjusted marginal proportions
# ------------------------------
emm <- emmeans(mod_stunting, ~ intv,lmer.df = "Kenward-Roger")
emm_resp <- regrid(emm)
...
summary(RD, infer = TRUE)

RD_sum <- summary(RD, infer = TRUE)
RD_sum$estimate_pp <- RD_sum$estimate * 100
RD_sum$LCL_pp      <- RD_sum$asymp.LCL * 100
RD_sum$UCL_pp      <- RD_sum$asymp.UCL * 100
...
# Likelihood Ratio Test (overall p-value for intv effect)
anova(null_mod, mod_sevstunting, test = "Chisq",ddf = "Kenward-Roger")

# ------------------------------
# Get adjusted marginal proportions
# ------------------------------
emm <- emmeans(mod_sevstunting, ~ intv,lmer.df = "Kenward-Roger")
emm_resp <- regrid(emm)
...
# Get adjusted marginal proportions
# ------------------------------
emm <- emmeans(mod_underweight, ~ intv)
emm_resp <- regrid(emm)
...
# Get adjusted marginal proportions
# ------------------------------
emm <- emmeans(mod_severe_underweight, ~ intv)
emm_resp <- regrid(emm)
...
emm <- emmeans(mod_severe_wasting, ~ intv)
emm_resp <- regrid(emm)
...
anova(mod_weight_gain, null_mod_weight_gain,ddf = "Kenward-Roger")
anova(mod_length_gain, null_mod_length_gain,ddf = "Kenward-Roger")
summary(mod_length_gain)
summary(mod_weight_gain)

emm_weight_gain_g_per_day <- emmeans(mod_weight_gain, ~ intv,lmer.df = "Kenward-Roger")
emm_weight_gain_g_per_day
summary(emm_weight_gain_g_per_day, infer = TRUE)

contr_weight_gain_g_per_day <- contrast(
  emm_weight_gain_g_per_day,
  method = "trt.vs.ctrl",
  ref = "intv1"  # placebo
)

summary(contr_weight_gain_g_per_day, infer = TRUE)


emm_length_gain_g_per_day <- emmeans(mod_length_gain, ~ intv,lmer.df = "Kenward-Roger")
emm_length_gain_g_per_day
summary(emm_length_gain_g_per_day, infer = TRUE)

contr_length_gain_g_per_day <- contrast(
  emm_length_gain_g_per_day,
  method = "trt.vs.ctrl",
  ref = "intv1"  # placebo
)

summary(contr_length_gain_g_per_day, infer = TRUE)
...
emm_weight_gain_g_per_day <- emmeans(mod_weight_gain, ~ intv)
emm_weight_gain_g_per_day
summary(emm_weight_gain_g_per_day, infer = TRUE)
...
emm_length_gain_g_per_day <- emmeans(mod_length_gain, ~ intv)
emm_length_gain_g_per_day
summary(emm_length_gain_g_per_day, infer = TRUE)
